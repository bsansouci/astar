...
function findPath(space, start, end){
  ...
  fScore[hashPoint(start)] = gScore[hashPoint(start)] + Astar.heuristicEstimate(start, end);
  ...
  while(openSet.length > 0) {
    ...
    for (var n in allNeighbours){
      ...
      if(!contains(openSet, neighbour, comparePoints) || tentativeGScore < gScore[neighbourHash]) {
        ...
        fScore[neighbourHash] = gScore[neighbourHash] + Astar.heuristicEstimate(neighbour, end);
